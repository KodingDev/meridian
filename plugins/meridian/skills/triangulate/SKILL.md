...
2. **Dispatch the `meridian:triangulate` agent** via the `Agent` tool with `subagent_type: meridian:triangulate`. Prompt body: the claim text, candidate source paths/types (whatever the orchestrator can identify — IDA EAs, script paths, config files, doc URLs), and the Ground Truth Audit format below. The agent's system prompt already contains the HARD-GATE.
...
5. **Write the full audit file** to `.meridian/audits/YYYY-MM-DD-<claim-slug>.md`. Slug rules: lowercase, ASCII-fold, non-alphanumeric → hyphen, collapse, trim, truncate to 60 chars. On filename collision, append `-2`, `-3`. Audits are append-only — never overwrite an existing audit.
